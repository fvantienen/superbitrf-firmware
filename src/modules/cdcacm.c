...
u8 cdacm_usbd_control_buffer[256];
...
bool cdcacm_send(const char *data, const int size) {
	...
	while ((size - (i * 64)) > 64) {
		while (usbd_ep_write_packet(cdacm_usbd_dev, 0x1, (data + (i * 64)), 64) == 0);
		i++;
	}

	while (usbd_ep_write_packet(cdacm_usbd_dev, 0x1, (data + (i * 64)), size - (i * 64)) == 0);
